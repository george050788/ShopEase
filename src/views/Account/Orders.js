    ...
    <div>
      {
        orders?.length > 0 && <div className='md:w-[70%] w-full'>
          <div className='flex justify-between'>
            ...
            <select name="" id="" className='border-2 rounded-lg mb-4 p-2' value={selectedFilter} onChange={handleOnChange}>
                    ...
                    </div>
                    {
                      order?.orderStatus !== 'CANCELLED' && <>
                        <Timeline stepCount={getStepCount[order?.orderStatus]} />
                        {
                          ...
                          <button onClick={() => onCancelOrder(order?.id)} className='bg-black h-[42px] w-[120px] text-white rounded-lg'>Cancel Order</button>
                        }
                      </>
                    }
                  </div>
